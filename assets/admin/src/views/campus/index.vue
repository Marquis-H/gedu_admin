<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.infomation')" align="center" min-width="150">
        <template slot-scope="scope">
          <span>{{scope.row.infomation}}</span>
        </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%">
      ...
      >
        ...
        <el-form-item :label="$t('table.infomation')" prop="infomation">
          <el-input v-model="temp.infomation" type="textarea" :rows="5"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getCampusList(this.listQuery).then(res => {
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          createCampus(this.temp).then(res => {
            if (res.code == 100) {
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateCampus(tempData).then(res => {
            if (res.code == 100) {
              var message = res.data;
              this.showErrorMessage(message);
            } else {
